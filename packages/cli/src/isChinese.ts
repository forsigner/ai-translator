...
export const isChinese = function (
  str: string,
  { includePunctuation = true }: { includePunctuation?: boolean } = {},
) {
...
export function isChineseSentence(str: string, probability = 60) {
  let truthyCount = 0
  for (const s of str) {
    if (isChinese(s)) truthyCount++
  }

  return (truthyCount / str.length) * 100 >= probability
}
